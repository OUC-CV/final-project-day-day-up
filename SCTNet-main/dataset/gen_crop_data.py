...
def get_croped_data_per_scene(scene_dir, patch_size=128, stride=64):
    ...
    if len(ldr_file_path) < 3:
        raise FileNotFoundError(f"Not enough LDR files found in {scene_dir}. Expected at least 3, but found {len(ldr_file_path)}.")
    
    label_path = os.path.join(scene_dir, 'HDRImg.hdr')
    
    if not os.path.exists(label_path):
        raise FileNotFoundError(f"Label file not found in {scene_dir}. Expected HDR file at {label_path}.")
