...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        # 短路连接，当输入和输出维度不同时，通过 1x1 卷积调整维度
...
class HDRTransformer(nn.Module):
    def __init__(self, img_size=128, patch_size=1, in_chans=6,
        ...
        # 最后的卷积层，用于在拼接后调整维度
        self.conv_first = nn.Conv2d(512, embed_dim, 3, 1, 1)
    ...
    def forward(self, x):
       ...
       # 使用 ResNet-18 进行特征提取
        ...
        # 应用空间注意力模块
        f1 = self.att_module_l(f1) * f1
        f2 = self.att_module_h(f2) * f2
        f3 = self.att_module_h(f3) * f3
